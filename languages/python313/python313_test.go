...
func TestParseDirty(t *testing.T) {
	...
	if f := report.Findings[1]; f.Rule != "F821" || f.Severity != linter.SeverityError || f.Line != 5 || f.Column != 11 || f.EndLine != 5 || f.EndColumn != 24 {
		t.Errorf("finding[1] = %+v, want F821/error at 5:11-5:24 (undefined names surface to students)", f)
	}
